...
import traceback  # Provides detailed error messages for debugging

def proccess_pdf(pdf_bytes):
 try:
    # If pdf file is empty return none
...
def ask_question(pdf_bytes, question): 
 try:
    text_splitter, vectorstore, retriever = proccess_pdf(pdf_bytes) # Process the PDF
    if text_splitter is None:
        return None  # No PDF uploaded    
    result = rag_chain(question, text_splitter, vectorstore, retriever) # Return the results with RAG
    return {result}
 except Exception as e:
        error_message = f"Error in ask_question: {str(e)}\n{traceback.format_exc()}"
        print(error_message)  # Log error to console
        return error_message  # Return error message to UI
